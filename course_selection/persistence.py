...
class WorkspaceDatabase:
    ...
    def _insert_profile(self, payload: dict[str, Any]) -> str:
        ...
        self.connection.execute(
            "insert into profiles values(?,?,?)", (identity, utc_now(), json.dumps(payload, ensure_ascii=False))
        )
        return identity
    ...
    def _insert_notice(self, payload: dict[str, Any]) -> str:
        ...
        self.connection.execute(
            ...
            (identity, payload.get("source_url", ""), payload.get("content_hash", ""), payload.get("status", "candidate"), utc_now(), json.dumps(payload, ensure_ascii=False)),
        )
        return identity
    ...
    def _insert_snapshot(self, kind: str, term: str, payload: dict[str, Any], source: str, profile_id: str | None = None, notice_id: str | None = None) -> str:
        ...
        self.connection.execute(
            ...
            (identity, kind, term, profile_id, notice_id, source, payload.get("source_at", now), now, json.dumps(payload, ensure_ascii=False)),
        )
        return identity
    ...
    def publish_snapshot(self, kind: str, term: str, payload: dict[str, Any], *, source: str, profile_id: str | None = None, notice_id: str | None = None) -> dict[str, Any]:
        with self.connection:
            ...
            old = self.connection.execute("select id from snapshots where kind=? order by created_at desc limit -1 offset 20", (kind,)).fetchall()
    ...
    def record_failed_attempt(self, kind: str, error: str) -> None:
        with self.connection:
            self.connection.execute("insert into refresh_attempts values(?,?,?,?,?,?,null)", (uuid.uuid4().hex, kind, "failed", utc_now(), utc_now(), error[:1000]))
