...
发布后使用可见浏览器完成一次低风险验收：确认离线启动不访问学校、连接后固定业务页验证成功、已有适用快照不会自动全量刷新、强制刷新会明确确认、活动任务期间其他远程按钮禁用，以及选课执行只对已确认的单个 `rwh` 提交一次。若服务端返回未知提示，确认同一教学班被阻断且没有自动重试。
...
基础毕业参考使用显式选择且不可原地修改的 `basic-graduation-reference-v1`。选择、认定学分申报、课程标签、外专业体系和队列预计均为本地操作；切换基线后旧报告保留为历史，只有显式“同步毕业进度”才访问教务并生成新版报告。面板中的“已满足/未满足/未知”只描述当前证据；用户申报和规划只会产生“预计可满足”。本版不含专业必修总量、完整培养方案审核或学校正式毕业认定，须人工核对个人培养方案。更换身份或清除个人工作区会清除这些个人状态。
