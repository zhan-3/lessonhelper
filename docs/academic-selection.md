...
数据只保存到 `.private/academic-selection/`。当前页面不会执行选课、退课或提交操作。待选课程快照会同时保存页面提供的 `saveXsxk` 教学班身份（`action_rwh`）和动作名称；只有存在该真实身份的课程才标记为 `execution_ready`，由课程代码和名称拼接的展示回退身份不可用于执行。
