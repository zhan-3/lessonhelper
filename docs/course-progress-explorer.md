...
1. 打开期末成绩查询；
2. 动态读取所有学期；
3. 逐学期查询全部课程记录，并在本地根据最终成绩区分“已通过”“未通过”和“已选待成绩”；
4. 遍历每个学期的全部分页；
5. 排除必修、去重并按指南基线计算已确认进度；“已选待成绩”仅参与预计学分，不计为已确认；
...
如果使用持久化选课工作台，也可以直接点击“同步毕业进度”。工作台会复用唯一的可见 Chromium 会话，读取所有学期和分页后，把不含具体成绩的进度报告作为 `progress` 快照写入 SQLite；同步失败时保留上一次完整快照。首次同步前，工作台仍会兼容显示独立 `collect` 命令生成的 `progress-report.json`。
